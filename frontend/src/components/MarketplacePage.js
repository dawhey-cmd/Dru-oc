...
import { Search, Download, Star, Smartphone, Globe, Apple, Check } from 'lucide-react';
  ...
  return (
    <div data-testid="marketplace-page">
      <div className="step-header">
        ...
        <p>Browse community-built skills, iOS integrations, and third-party plugins</p>
      </div>
      ...
      <div className="panel" style={{ display: 'flex', alignItems: 'center', gap: 16, borderColor: 'rgba(0,229,255,0.2)', background: 'rgba(0,229,255,0.03)' }}>
        <div style={{ width: 48, height: 48, borderRadius: 12, background: 'linear-gradient(135deg, #007AFF, #5856D6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Smartphone size={22} color="#fff" />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600, color: '#fff', fontSize: '0.95rem' }}>
            iPhone / iOS Integration Available
          </div>
          <div style={{ fontSize: '0.82rem', color: '#8892A8', marginTop: 2 }}>
            {iosCount} skills support iOS — Siri Shortcuts, widgets, iCloud sync, HomeKit, and more.
            Install on Windows, pair with your iPhone.
          </div>
        </div>
      ...
      <div className="panel" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          ...
          <input className="input-field" data-testid="marketplace-search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search marketplace..." style={{ paddingLeft: 36 }} />
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {['All', ...marketplace.categories].map(cat => (
          <button key={cat} data-testid={`mkt-cat-${cat.toLowerCase()}`} className={`btn ${filterCat === cat ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setFilterCat(cat)} style={{ padding: '5px 12px', fontSize: '0.78rem' }}>
            {cat}
          </button>
        ))}
      </div>
      ...
      {/* Skills Grid */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: 40, color: '#5A6480' }}>Loading marketplace...</div>
      ) : (
              ...
              <div key={skill.id} data-testid={`mkt-skill-${skill.id}`} className={`skill-card ${installed ? 'selected' : ''}`}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: '#FFD54F' }}>{skill.rating}</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
                  {skill.platform?.map(p => (
                    <span key={p} style={{
                      ...
                      background: p === 'ios' ? 'rgba(0,122,255,0.15)' : 'rgba(255,255,255,0.05)',
                      color: p === 'ios' ? '#4FC3F7' : '#5A6480',
                    }}>
                      {p}
                    </span>
                  ))}
      ...
      {filtered.length === 0 && !loading && (
        <div style={{ textAlign: 'center', padding: 40, color: '#5A6480' }}>No marketplace skills found matching your filters</div>
      )}
    </div>
  );
}
