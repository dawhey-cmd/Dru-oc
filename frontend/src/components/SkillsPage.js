...
function SkillsPage({ config, updateConfig, goNext, goPrev, skills }) {
  ...
  const selectPopular = () => {
    const popular = skills.filter(s => s.popular).map(s => s.id);
    updateConfig('selected_skills', popular);
  };
  ...
  return (
    <div data-testid="skills-page">
      <div className="step-header">
        <h1>Select Skills</h1>
        <p>Choose the skills and plugins to install with OpenClaw. You can always add more later.</p>
      </div>
      ...
      <div className="panel" style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <button className="btn btn-secondary" data-testid="select-popular-btn" onClick={selectPopular}>
          Select Popular
        </button>
        <button className="btn btn-ghost" data-testid="clear-skills-btn" onClick={clearAll}>
          Clear All
        </button>
        <div style={{ fontSize: '0.82rem', color: '#8892A8' }}>
          <span style={{ color: '#FF2D2D', fontWeight: 600 }}>{config.selected_skills.length}</span> selected
        </div>
      </div>
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                ...
                {isSelected && (
                  <div style={{
                    width: 20, height: 20, borderRadius: '50%', background: '#FF2D2D',
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: 40, color: '#5A6480' }}>
          No skills found matching "{search}"
        </div>
      )}

      <div className="step-nav">
        <button className="btn btn-ghost" data-testid="skills-back-btn" onClick={goPrev}>
          <ArrowLeft size={16} /> Back
        </button>
        <button className="btn btn-primary" data-testid="skills-next-btn" onClick={goNext}>
